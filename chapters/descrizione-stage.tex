...
\begin{risk}{Integrazione tra componenti eterogenei}
    \vspace{0.5em}
    \riskdescription{L'integrazione tra componenti sviluppati in ambienti e linguaggi differenti (\emph{\gls{Javag}\glsfirstoccur} per il motore di reportistica e \emph{\gls{pythong}\glsfirstoccur} per il server \emph{\gls{restg}\glsfirstoccur}) potrebbe generare problemi di comunicazione, gestione degli errori o incompatibilità nei formati dei dati scambiati}
    \vspace{0.5em}
    \riskimpact{Alto}
    \vspace{0.5em}
    \riskprobability{Media}
    \vspace{0.5em}
    \risksolution{Definizione di interfacce \emph{\gls{restg}} standardizzate, utilizzo di formati di scambio dati strutturati (\emph{\gls{JSONg}\glsfirstoccur}) e \emph{test} incrementali delle \emph{\gls{apig}\glsfirstoccur} per verificare il corretto funzionamento delle interazioni tra i componenti}
...
\begin{risk}{Dipendenza da tecnologie di terze parti}
    \vspace{0.5em}
    \riskdescription{L'utilizzo di \emph{framework} e librerie di terze parti (\emph{JasperReports}, \emph{BIRT Runtime}, \emph{\gls{Javag}}, \emph{\gls{pythong}}) introduce una dipendenza da tecnologie esterne, con possibili problemi di compatibilità, supporto o aggiornamenti futuri}
    \vspace{0.5em}
    \riskimpact{Medio}
    \vspace{0.5em}
...
Le fasi specifiche del progetto sono state le seguenti:
\begin{itemize}
...
Nella tabella \ref{tab:obiettivi} sono elencati gli obiettivi dello \emph{stage}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|l|}
        ...
        OB1 & Studio documentazione e analisi comparativa \\ \hline
        OB2 & Valutazione e implementazione \emph{\gls{PoCg}\glsfirstoccur} di due soluzioni alternative \\ \hline
        OB3 & Documentazione configurazione \\ \hline

        \rowcolor{gray!40}
        \multicolumn{2}{|c|}{\textbf{Obiettivi Desiderabili}} \\ \hline
        DE1 & Analisi costi e licenze \\ \hline
        DE2 & Raccolta feedback con tutor o key user da lui individuati \\ \hline
        DE3 & Definizione best practice di integrazione \\ \hline

        \rowcolor{gray!40}
        \multicolumn{2}{|c|}{\textbf{Obiettivi Facoltativi}} \\ \hline
        FA1 & Valutazione altre alternative \\ \hline
        FA2 & Integrazione di funzioni di reporting personalizzato \\ \hline
    \end{tabular}
\end{table}
